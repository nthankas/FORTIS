...
| `test/test_safety_arm_integration.py` | Placeholders (`@pytest.mark.skip`) for the planned safety + arm contract: IDLE rejects MoveToPose, ARM_AT_VIEW accepts, ORBIT rejects, FAULT forces rejection. Bodies land alongside the MoveIt 2 wrapper for `fortis_arm`. |
