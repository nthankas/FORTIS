...
| `test/test_safety_arm_integration.py` | Brings up `mission_state_node` (fortis_safety) and `arm_controller` (fortis_arm) in one launch and exercises the gripper-service gating contract end to end. Verifies (a) `IDLE`, `ORBIT`, `TARGETING`, and `FAULT` reject both `open_gripper`/`close_gripper` with the `rejected: state ...` message; (b) `ARM_AT_VIEW`, `INSPECT`, `PICK`, `HOLDING`, and `RETURN_HOME` accept the gate and return the `gripper actuation not implemented` stub message; (c) the `ALLOWED_ARM_STATES` allowlist imported from `fortis_arm` matches the per-state expectations. |
