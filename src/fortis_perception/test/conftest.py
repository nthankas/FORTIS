"""
Per-process pytest setup for fortis_perception.
...
ROS_DOMAIN_ID those processes share one DDS domain: a co-running
package's latched/streaming topics can then leak into this package's
subscribers and flip an assertion -- the intermittent cross-talk flake
that passes when each package is tested alone.
