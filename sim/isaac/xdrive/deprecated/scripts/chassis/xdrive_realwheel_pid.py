    ...
    for ri, barrel in enumerate(src_parts["roller_barrel"]):
        ...
        if ri == 0:
            ...
            print(f"  Roller chain: {N_SUB} spheres r={sub_r*1000:.1f}mm "
                  ...
                  f"(outer center @ {outer_off*1000:.1f}mm, "
                  f"chord={chord*1000:.1f}mm)", flush=True)
...
if headless and args.orbit_speed is not None:
    ...
    out_csv = args.orbit_csv or os.path.join(
        RESULTS_ROOT, "orbit_realwheel_pid",
    ...
    # PID orbit controller: feedforward + Kp position error + Kw yaw error,
    # with a ramp-in so wheels don't lurch on the first frame.
    RAMP_S = 1.5
    K_pos = 0.4
    K_yaw = 1.5
    V_MAX_FACTOR = 1.3
    WZ_MAX_FACTOR = 2.5
    print(f"Tracker (PID): K_pos={K_pos}, K_yaw={K_yaw}, ramp={RAMP_S}s, "
          f"vmax={V_MAX_FACTOR}xspeed", flush=True)
    va = np.zeros(ndof)
    ...
    for frame in range(total_frames):
        ...
        # Smooth ramp on omega: phase = integral of omega_eff(tau) dtau.
        if t < RAMP_S:
            omega_eff = omega * t / RAMP_S
            phase_t = init_phase + 0.5 * omega * t * t / RAMP_S
        else:
            omega_eff = omega
            phase_t = init_phase + 0.5 * omega * RAMP_S + omega * (t - RAMP_S)

        tgt_x = r0 * math.cos(phase_t)
        tgt_y = r0 * math.sin(phase_t)
        tgt_vx = -r0 * omega_eff * math.sin(phase_t)
        tgt_vy =  r0 * omega_eff * math.cos(phase_t)
        ...
        # Yaw correction: desired yaw = phase_t (chassis-X tracks radial out).
        yaw_err = phase_t - yaw_now
        while yaw_err > math.pi:
            yaw_err -= 2.0 * math.pi
        while yaw_err < -math.pi:
            yaw_err += 2.0 * math.pi
        wz = omega_eff + K_yaw * yaw_err
        wz_cap = WZ_MAX_FACTOR * abs(omega) + 0.5
        if wz > wz_cap:  wz = wz_cap
        if wz < -wz_cap: wz = -wz_cap

        tv = xdrive_ik(vx_chs, vy_chs, wz)
        for ii, di in enumerate(drive_dof_indices):
            va[di] = tv[ii]
