"""
FORTIS X-drive — V2: Kaya omni wheel with CAPSULE roller colliders.

Same chassis + wheel layout as canonical/xdrive_realwheel.py. Each roller
collider is a UsdGeom.Capsule sized from the actual scaled barrel mesh
bounding box and oriented along the roller's tangent axis.

Why capsule (not the full barrel mesh): the source barrel mesh has 592
verts per side. Convex-hull collision on this mesh blew up PhysX broad-
phase (verified -- the chassis ejected to 1.5e8 m on the first step
with an avalanche of "Illegal BroadPhaseUpdateData" errors). A capsule
gives the same continuous-contact rolling arc along the cylindrical
side of the roller -- which is what kills the discrete-sphere stepping
problem v1 has -- without the broken hull cook.

Original v1 (sphere collider) is retained at canonical/xdrive_realwheel.py.

Usage: IsaacSim\\python.bat xdrive_realwheel_v2.py --gui
       IsaacSim\\python.bat xdrive_realwheel_v2.py --orbit-speed 0.2
"""
...
parser.add_argument("--orbit-speed", type=float, default=None,
                    help="Headless orbit test at this speed (m/s). Implies "
                         "--reactor. Records torques + state to CSV.")
...
if args.orbit_speed is not None:
    args.reactor = True       # orbit only makes sense in the reactor
    args.headless = True      # headless for sweep efficiency
if args.tunnel:
...
# Mass — total robot 40 lb hard cap (arm + chassis + wheels combined).
#   chassis frame + arm-stowed-lumped : 14.144 kg
#   4 wheels @ 1.0 kg each            :  4.000 kg
#   Total physics mass                : 18.144 kg = 40 lb
...
def build_omniwheel(stage, wheel_path, src_parts, src_center, wheel_mat, wname):
    ...
    roller_coll_r_global = max(min(safe_roller_r, max_surface_r), 0.005)  # unused in v2
    # V2: collider is convex-decomposition of barrel mesh per roller.
    print(f"  Roller collision (V2): convex-decomposition mesh per roller "
          f"(maxHulls=8, vertLimit=32, "
          f"min_pair_dist={min_pair_dist*1000:.1f}mm)", flush=True)
    ...
    for ri, barrel in enumerate(src_parts["roller_barrel"]):
        ...
        # V2: Convex DECOMPOSITION of the actual scaled barrel mesh.
        # PhysX builds multiple smaller convex hulls that together cover
        # the mesh shape -- avoids the single-hull broadphase failure
        # mode and gives a much closer fit to the real barrel curve than
        # a sphere or capsule.
        bmp = rp + "/barrel_decomp"
        barrel_coll_mesh = create_mesh_prim(stage, bmp, barrel, src_center,
                         SCALE_UNIFORM, SCALE_UNIFORM, SCALE_UNIFORM,
                         body_offset=scaled_center, color=roller_color)
        UsdPhysics.CollisionAPI.Apply(barrel_coll_mesh.GetPrim())
        UsdPhysics.MeshCollisionAPI.Apply(barrel_coll_mesh.GetPrim()) \
            .CreateApproximationAttr().Set("convexDecomposition")
        UsdShade.MaterialBindingAPI.Apply(barrel_coll_mesh.GetPrim()).Bind(
            wheel_mat, UsdShade.Tokens.weakerThanDescendants, "physics")
        pcoll = PhysxSchema.PhysxCollisionAPI.Apply(barrel_coll_mesh.GetPrim())
        pcoll.CreateContactOffsetAttr(0.001)
        pcoll.CreateRestOffsetAttr(0.0005)
        # Limit decomposition complexity so cooking is bounded
        pdec = PhysxSchema.PhysxConvexDecompositionCollisionAPI.Apply(
            barrel_coll_mesh.GetPrim())
        pdec.CreateMaxConvexHullsAttr(8)
        pdec.CreateHullVertexLimitAttr(32)
...
# ============================================================================
print("=" * 60, flush=True)
print("FORTIS X-Drive V2 (Real Omni Wheel Meshes, mesh-collider rollers)", flush=True)
print("=" * 60, flush=True)
...
if headless and args.orbit_speed is not None:
    import csv as _csv
    ...
    out_csv = args.orbit_csv or os.path.join(
        RESULTS_ROOT, "orbit_realwheel_v2",
        f"orbit_realwheel_v2_{speed:.2f}mps.csv")
    ...
    # Position-tracking controller: at each step, compute the target point
    # on the orbit at time t, transform world-frame error into chassis-frame
    # velocity command, send through xdrive_ik. Yaw drift is tolerated
    # because the world->chassis transform uses the current yaw each step.
    K_pos = 0.5     # 1/s gain on world-position error (m/s per m of error)
    V_MAX_FACTOR = 1.4   # cap chassis-frame |v| at K * commanded speed
    print(f"Tracker gains: K_pos={K_pos}, vmax={V_MAX_FACTOR}xspeed", flush=True)
    va = np.zeros(ndof)
    # initial orbit-angle phase (so target starts at current chassis position)
    ...
    for frame in range(total_frames):
        ...
        # Target point on the orbit at time t (CCW from initial phase).
        phase_t = init_phase + omega * t
        tgt_x = r0 * math.cos(phase_t)
        tgt_y = r0 * math.sin(phase_t)
        tgt_vx = -r0 * omega * math.sin(phase_t)   # world-frame target vel
        ...
        # World-frame velocity command = feedforward + Kp * pos error
        ex = tgt_x - pos_now[0]
        ey = tgt_y - pos_now[1]
        ...
        # Rotate world->chassis (undo current yaw).
        cy = math.cos(-yaw_now)
        sy = math.sin(-yaw_now)
