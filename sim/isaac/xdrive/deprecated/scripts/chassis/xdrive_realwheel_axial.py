...
parser.add_argument("--hz", type=int, default=480,
                    help="Physics Hz (360-480, default 480)")
...
def build_omniwheel(stage, wheel_path, src_parts, src_center, wheel_mat, wname):
    ...
    for ri, barrel in enumerate(src_parts["roller_barrel"]):
        ...
        # GPU PhysX limits convex hulls to 64 verts -- mesh collision
        # fails. Convex hull / decomposition / capsule on the actual mesh
        # all blew up the broadphase (verified). A sphere CHAIN gives a
        # near-continuous contact arc (each sphere overlaps neighbours in
        # the chain) and lives entirely on the same revolute-jointed
        # roller body, so omni anisotropic rolling is preserved.
        roller_center_dist = float(np.sqrt(scaled_center[0]**2 + scaled_center[2]**2))
        # Tangent direction in wheel frame (= roller body local frame).
        theta = math.atan2(scaled_center[2], scaled_center[0])
        tang_x = -math.sin(theta)
        tang_z =  math.cos(theta)
        # Inter-roller chord (NUM_ROLLERS evenly spaced around the wheel):
        chord = 2.0 * roller_center_dist * math.sin(math.pi / NUM_ROLLERS)

        N_SUB = 5
        # Per-sphere radius: chord/4 leaves room for spread and adjacent-
        # roller clearance. Each sphere significantly overlaps its
        # in-chain neighbours so contact along tangent is continuous.
        sub_r = max(0.005, chord * 0.25)
        # Spacing such that outermost sphere edge sits at chord/2 from
        # the roller center, leaving the same ~4 mm clearance to the
        # adjacent roller's outermost sphere as the original safe radius.
        spacing = max(0.0, (chord - 2.0 * sub_r) / float(N_SUB - 1))
        # Cap radius vs wheel-surface so spheres don't poke past the rim.
        sub_r = min(sub_r, float(WHEEL_RADIUS - roller_center_dist) + 0.002)

        # AXIAL VARIANT: same 5-sphere tangent chain, plus two extra rows
        # offset along the wheel's hub axis (Y in wheel frame). Captures
        # the barrel's axial extent so contact patch is closer to the
        # full barrel surface, not just its equator line.
        # Y offset: scaled barrel half-extent in the source Y direction,
        # capped so axial spheres stay inside the wheel width and don't
        # interfere with the hub sideplate.
        bx_pts = np.array(barrel["points"])
        center_b = bx_pts.mean(axis=0)
        rel_b = bx_pts - center_b
        y_extent_unscaled = float(rel_b[:, 1].max() - rel_b[:, 1].min())
        y_offset = min(WHEEL_WIDTH * 0.25,
                       y_extent_unscaled * SCALE_Y * 0.4)
        y_rows = [-y_offset, 0.0, +y_offset]   # 3 axial rows

        sphere_idx = 0
        for yo in y_rows:
            for si in range(N_SUB):
                off = (si - (N_SUB - 1) / 2.0) * spacing
                smp = rp + f"/collider_{sphere_idx}"
                sphere_idx += 1
                ...
                tx = float(off * tang_x)
                tz = float(off * tang_z)
                if abs(tx) > 1e-9 or abs(yo) > 1e-9 or abs(tz) > 1e-9:
                    sxf.AddTranslateOp().Set(Gf.Vec3d(tx, float(yo), tz))
        ...
        if ri == 0:
            ...
            print(f"  Roller grid (axial): {len(y_rows)}x{N_SUB} spheres "
                  f"r={sub_r*1000:.1f}mm spacing={spacing*1000:.1f}mm "
                  f"y_off=±{y_offset*1000:.1f}mm", flush=True)
...
if headless and args.orbit_speed is not None:
    ...
    out_csv = args.orbit_csv or os.path.join(
        RESULTS_ROOT, "orbit_realwheel_axial",
