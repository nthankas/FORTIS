"""
FORTIS arm collision-free torque sweep v3 -- headless Isaac Sim.

Single-config: 30" CF arm, always loaded with 3 lb payload, J1 fixed at 0
during the sweep. Statistical method: structured Cartesian grid sweep
over J2/J3/J4 (continuous joints in v3 hardware -- full 360 deg ranges
on J2/J3, +/-100 on J4).

Pipeline matches v2:
  Stage 1 (this script): collision-free physics sweep -- teleport to each
    pose, PD-hold, measure joint efforts. Output: raw CSV with every pose.
  Stage 2 (arm_sweep_filter_v3.py): apply heuristic geometry filters
    (chassis collision, self-collision, floor) plus the new poloidal
    reactor cross-section filter, plus per-pose tipping margins.

Imports arm_ik_v3 for analytical cross-check.

Usage:
  IsaacSim\\python.bat tools/arm_continuous_sweep_v3.py --step
  IsaacSim\\python.bat tools/arm_continuous_sweep_v3.py --step --coarse --test
"""
...
parser = argparse.ArgumentParser(description="FORTIS arm continuous sweep v3 -- headless")
parser.add_argument("--step", action="store_true",
                    help="Use the 4.5\" step environment (recommended for sizing)")
...
parser.add_argument("--fine", action="store_true",
                    help="Fine grid (5/5/10 deg) -- ~62k poses, hours")
parser.add_argument("--coarse", action="store_true",
                    help="Coarse grid (20/30/45 deg) -- ~648 poses, ~10-15 min")
...
# ============================================================================
# Arm constants (V3, single-config: 30" CF, loaded)
# ============================================================================
J1_STACK_H = 1.5 * IN

# Link lengths (V3 fixed; matches arm_ik_v3.py)
L_L2_IN = 14.168
L_L3_IN = 11.222
L_L4_IN = 4.970
L_L2 = L_L2_IN * IN
L_L3 = L_L3_IN * IN
L_L4 = L_L4_IN * IN
ARM_REACH_IN = L_L2_IN + L_L3_IN + L_L4_IN  # ~30.36"

# V3 per-joint masses (kg)
M_J1 = 0.580
M_J2 = 2.665
...
# V3 CF tube: square hollow 1.128"x1.128" OD, 1.000"x1.000" ID, 0.064" wall
# Linear density: 1.03 lb / 72 in -> 0.01431 lb/in -> 0.2557 kg/m
...
# V3 gripper: ServoCity parallel kit + D645MW (lighter assembly than v2)
M_GRIPPER_BARE = 0.133
M_PAYLOAD = 1.361
M_GRIPPER = M_GRIPPER_BARE + M_PAYLOAD  # always loaded in v3
ARM_LOADED = True

# V3 camera: OAK-D Pro on L4 midpoint (1.5" from J4)
M_CAMERA = 0.091
CAM_X_ON_L4 = 1.5 * IN

# V3 square tube cross-section
CF_TUBE_OD = 1.128 * IN
...
# V3 joint limits: J1/J2/J3 continuous (steppers), J4 servo +/-101 deg
ARM_LIMITS_DEG = {
    "ArmJ1": (-180.0, 180.0),
    "ArmJ2": (-180.0, 180.0),
    "ArmJ3": (-180.0, 180.0),
...
# ============================================================================
# Sweep parameters (V3 -- continuous joints, full 360 on J2/J3)
# ============================================================================
# Statistical method = structured Cartesian grid (uniform deterministic
# coverage of the joint space). Default ~3.9k poses; coarse for quick
# tests, fine for paper-quality data.
if args.fine:
    J2_STEP, J3_STEP, J4_STEP = 5, 10, 20    # ~26k poses
elif args.coarse:
    J2_STEP, J3_STEP, J4_STEP = 20, 30, 45   # ~648 poses
else:
    J2_STEP, J3_STEP, J4_STEP = 15, 20, 25   # ~3.9k poses

# J2 / J3: continuous-rotation steppers, full 360 deg
J2_MIN, J2_MAX = -180, 180 - J2_STEP   # avoid double-counting endpoint
J3_MIN, J3_MAX = -180, 180 - J3_STEP
# J4: D845WP servo, +/-101 deg hardware limit (rounded to +/-100 for the grid)
J4_MIN, J4_MAX = -100, 100
...
def build_arm(stage, robot_path, chassis_path):
    ...
    print(f"\nARM v3: 30\" CF, LOADED, total={total_arm_mass:.3f}kg", flush=True)
...
def compute_analytical_torques(j2_usd_deg, j3_usd_deg, j4_usd_deg):
    """Compute gravity torques from USD joint angles using arm_ik_v3 FK."""
...
config_tag = f"30in_loaded_{env_label}_v3"
