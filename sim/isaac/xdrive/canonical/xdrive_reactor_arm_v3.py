"""
FORTIS X-drive chassis + 4-DOF parallel-link arm — V3 HARDWARE (2026-05-03).

Single-config build: 30-inch carbon-fiber arm, always loaded with 3 lb
payload. No --24arm / --36arm / --metal / --armloaded flags. v2 remains
available alongside this script for back-comparison.
  ...
  J3 elbow   : NEMA 17 (590 g) + Cricket gearbox (80 g)
               + 0.593 lb AL connector (269 g)             (0.939 kg, 12 Nm)
  J4 wrist   : Hitec D845WP servo + metal mount
               (227 g + 0.709 lb)                          (0.5486 kg, 4.9 Nm)
  Links      : CF square hollow 1.128"x1.128" OD, 1.000"x1.000" ID,
               0.064" wall  (0.01431 lb/in = 0.2557 kg/m)
  L2 (J2->J3): 14.168"  (91.9 g tube)
  L3 (J3->J4): 11.222"  (72.8 g tube)
  L4 (J4->EE):  4.97"   (32.3 g tube)
  Camera     : OAK-D Pro (91 g) on L4 midpoint, 1.5" from J4
  End-effector: ServoCity parallel kit + D645MW (133 g total)
  Payload    : 3 lb (1361 g) at L4 tip (always loaded)
...
Usage: IsaacSim\\python.bat canonical/xdrive_reactor_arm_v3.py --gui
       IsaacSim\\python.bat canonical/xdrive_reactor_arm_v3.py --gui --reactor
       IsaacSim\\python.bat canonical/xdrive_reactor_arm_v3.py --gui --step
"""
...
# ============================================================================
# Constants — 4-DOF arm (V3 single-config: 30 in CF, loaded)
# ============================================================================
...
# Link lengths (V3, fixed) — match arm_ik_v3.py
L_L2_IN = 14.168
L_L3_IN = 11.222
L_L4_IN = 4.970
L_L2 = L_L2_IN * IN
...
# CF tube: square hollow 1.128" x 1.128" OD, 1.000" x 1.000" ID, 0.064" wall.
...
LINK_MATERIAL = "carbon_fiber_v3_square"
M_L2 = CF_DENSITY_KG_PER_M * L_L2
M_L3 = CF_DENSITY_KG_PER_M * L_L3
M_L4 = CF_DENSITY_KG_PER_M * L_L4

# End-effector tip mass
# V3 gripper total: ServoCity parallel kit + D645MW assembled lighter (133 g)
M_GRIPPER_BARE = 0.133
M_PAYLOAD = 1.361         # 3 lb max payload (always loaded in v3)
M_GRIPPER = M_GRIPPER_BARE + M_PAYLOAD
ARM_LOADED = True         # v3 is always loaded; flag kept for downstream prints

# Depth camera: OAK-D Pro, 91 g, mounted on L4 at its midpoint
# (1.5" from the J4 pivot, same location as v2).
M_CAMERA = 0.091
CAM_X_ON_L4 = 1.5 * IN

# CF tube cross-section: square 1.128" x 1.128"
CF_TUBE_OD = 1.128 * IN
CF_TUBE_Y = CF_TUBE_OD
CF_TUBE_Z = CF_TUBE_OD
CF_TUBE_SIZE_MAX = CF_TUBE_OD
...
# Joint limits (V3 hardware reality):
#   J1, J2, J3 are continuous-rotation steppers + gearboxes. Software limit
#   set to +/-180 deg; sim wraps freely past these bounds in practice.
#   J4 is a Hitec D845WP servo with 202 deg total travel (+/-101 deg).
ARM_LIMITS_DEG = {
    "ArmJ1": (-180.0, 180.0),
    "ArmJ2": (-180.0, 180.0),
    "ArmJ3": (-180.0, 180.0),
...
def build_arm(stage, robot_path, chassis_path):
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"ARM BUILD SUMMARY v3 (30\" CF, LOADED)", flush=True)
    print(f"{'='*60}", flush=True)
    print(f"  J1 stack height: {J1_STACK_H*1000:.0f}mm ({J1_STACK_H/IN:.2f}\")", flush=True)
    print(f"  L2={L_L2/IN:.3f}\"  L3={L_L3/IN:.3f}\"  L4={L_L4/IN:.3f}\"", flush=True)
    print(f"  Total reach: {(L_L2+L_L3+L_L4)/IN:.2f}\"", flush=True)
    print(f"  Joint masses:  J1={M_J1:.3f}  J2={M_J2:.3f}  J3={M_J3:.3f}  "
          f"J4={M_J4:.3f} kg", flush=True)
    print(f"  Link tube: CF square {CF_TUBE_OD/IN:.3f}\" OD, "
          f"{CF_DENSITY_LB_PER_IN*12.0:.3f} lb/ft -> {CF_DENSITY_KG_PER_M*1000:.1f} g/m", flush=True)
...
# ============================================================================
print("=" * 60, flush=True)
print(f"FORTIS X-Drive + 4-DOF Parallel Link Arm v3 "
      f"(30\" CF, LOADED with 3 lb payload)", flush=True)
print("=" * 60, flush=True)
