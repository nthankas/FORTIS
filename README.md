...
| `fortis_bringup` | `bringup.launch.py` composes `mission_state_node` + `drive_node` + `odrive_health_monitor_node`; `sim.launch.py` brings up `robot_state_publisher` + `joint_state_publisher` + `foxglove_bridge`; `teleop.launch.py` brings up `teleop_twist_keyboard` with `/cmd_vel` remap; `oak_chassis_front.launch.py` brings up the front OAK-D Lite (RGB + depth + RGBD cloud + IMU) via `depthai_ros_driver`. Remaining-camera and arm-controller includes pending. |
