# FORTIS Phase 2: Controlled Chassis Movement --  Prompt
...
### Token Efficiency Principle
 ...
 tokens are expensive. Use them ONLY for tasks that require automated iteration: parameter sweeps, multi-run simulations, data logging across many configurations, and report generation. Everything that can be done faster by the human clicking in the Isaac Sim GUI should be listed as a manual instruction, not scripted.

### Step 1: Human Manual Setup (no  tokens)

The human will do these steps in the Isaac Sim GUI before running any scripts.  should print these instructions clearly during Phase 0 and then wait for confirmation:
...
6. Confirm to  that the scene is ready.

### Step 2: Interactive Manual Control Script ( writes, human runs with GUI open)

Write `manual_drive.py` (see Goal 6 above). This is the FIRST script  produces because:
...
### Step 3: Rotation Test Sweep ( -- heavy compute, many runs)
...
### Step 4: Driving Test Sweep ( -- heavy compute, many runs)
...
### Step 5: Arm Stability Test ( -- multiple poses, data logging)
...
## Manual vs. Scripted Task Division
 ...
 tokens are limited. Only use them for multi-run automated work. The human is sitting at the RTX 4080 PC with Isaac Sim open.
...
** writes, human runs interactively (Step 2):**
...
** runs autonomously (Steps 3-6 -- this is where tokens are justified):**
